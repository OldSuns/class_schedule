...
import { parseWeekBasedField } from "./weekBasedFieldUtils";
...
export const getCourseLocation = (location, week) => {
  return parseWeekBasedField(location, week, "未排地点");
};

const buildDistinctLocationsForGroup = (courses, currentWeek) => {
  const seen = new Set();
  const result = [];
  for (const c of Array.isArray(courses) ? courses : []) {
    if (!c?.location) continue;
    const loc = getCourseLocation(c.location, currentWeek);
    if (!loc || loc === "未排地点" || seen.has(loc)) continue;
    seen.add(loc);
    result.push(loc);
  }
  return result;
...
export const getCourseNote = (note, week) => {
  return parseWeekBasedField(note, week, "");
};
  ...
  for (const day of DAYS) {
    ...
    while (period <= 13) {
      ...
      if (!cell.hasCurrentWeekCourse) {
        const distinctLocationsByDisplay = (cell.displayCourses ?? []).map(
          (dc) => {
            const audienceKey = getCourseAudienceKey(dc);
            const sameAudienceCourses = (cell.filteredCourses ?? []).filter(
              (c) => getCourseAudienceKey(c) === audienceKey
            );
            return buildDistinctLocationsForGroup(
              sameAudienceCourses,
              currentWeek
            );
          }
      ...
      const distinctLocationsByDisplay = displayCourses.map((dc) => {
        const audienceKey = getCourseAudienceKey(dc);
        const sameAudienceCourses = mergedCurrentWeekCourses.filter(
          (c) => getCourseAudienceKey(c) === audienceKey
        );
        return buildDistinctLocationsForGroup(sameAudienceCourses, currentWeek);
      });
